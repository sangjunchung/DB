...
FROM EMPLOYEE
WHERE ROWNUM > 5;
-- ROWNUM 은 무조건 1부터 시작하기 때문에 5 이후는 처음부터 나올수 없는 조건
-- 주로 랭킹에서 쓰임

-- ROWNUM을 이용해서 월급이 가장 높은 3명을 뽑으려했지만 되지 않음
...
NVL2 : NULL 이 아닌 컬럼의 값을 지정한 숫자 또는 문자로 변경해서 반환
[사용법]
NVL2(컬럼명,NULL이 아닌 경우 변경할 값, 컬럼 값이 NULL 일 때 변경할 값)
*/
...
-- 단 성과급이 없으면 0으로 표시
SELECT EMP_NAME,SALARY,SALARY*NVL(BONUS, '0')
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 사번, 이름, 전화번호 조회
-- 전화번호가 없으면 없음으로 표기
-- 전화번호가 있으먄 010******** 형식으로 변경해서 조회
