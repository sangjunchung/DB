...
/*
sqlplus : oracle db 에서 제공하는 명령줄 도구
sys : 시스템(system) 단축어
as sysdba : 시스템 데이터베이스 어드민 (System Datebase Admin)
설치할 때 작성한 비밀번호 작성하기
비밀번호는 눈에 보이지 않음
*/
...
-- SCRIPT TRUE로 변경해줘야 했지만 이제는 하지 않아도 됨
/*
...
/*
GRANT : 사용자에게 특정 권한이나 역활을 부여하기 위한 명령어
CONNECT : 사용자가 데이터베이스에 접속할 수 있는 권한 부여
    -> KH T 반에 들어올 수 있는 권한
RESOURCE : 테이블, 인덱스, 뷰 등을 생성할 수 있는 권한
    -> T 반에서 특정 행동이나 만들기, 변경, 설정 등 할 수 있는 권한 
이외 여러가지 권한 있음, 권한을 여러가지 한 번에 작성할 때는 ',' 를 사용해서 작성
TO 아이디명 : 어떤 아이디에 권한을 줄 것인지 설정
*/
GRANT RESOURCE, CONNECT TO 아이디명;
...
INSERT INTO MEMBER (M_ID, M_NAME, M_AGE, M_PHONE, M_DATE) VALUES (7, '김놀부', '60', '1234567896', '2024-05-07');
